 ...
 *
 * <p>Each {@code hardened*} test asserts the library blocks the payload; its {@code unconfigured*} positive control asserts the same payload parses once the
 * limit is disabled, so a block reflects the hardening rather than a broken wrapper. The library pins no custom entity-expansion limit; each parser keeps its own
 * secure-processing default, which varies by implementation: {@code 2,500} (stock JDK), {@code 64,000} (external Xerces under {@code FEATURE_SECURE_PROCESSING}),
 * {@code 100,000} (external Xerces' and Woodstox's own security managers). A payload therefore has to exceed the largest of these.</p>
 *
 ...
 * </ul>
 *
 * <p>Why a single character {@code "A"}: it makes the expanded size equal the expansion count, so a payload's size maps directly onto each parser's limit.</p>
 */
class BillionLaughsTest {

    /** 100,000 + 1,000 = 101,000 expansions; above every JVM parser's secure default (2,500 / 64,000 / 100,000). */
    ...
    private static String xsltPayload() {
        return withDoctype("xsl:stylesheet", AttackTestSupport.xsltBody(content()));
    }
